...
- The solution is `Codebelt.Extensions.Newtonsoft.Json.slnx`. Central package versions are maintained in `Directory.Packages.props`.
...
- `Codebelt.Extensions.Newtonsoft.Json.slnx` is the solution used for local development.
...
```powershell
dotnet restore "Codebelt.Extensions.Newtonsoft.Json.slnx"
dotnet build "Codebelt.Extensions.Newtonsoft.Json.slnx" --configuration Release --no-restore
```
...
The CI test plan currently runs **3** project(s) and excludes **0** project(s). The workflow also has an optional macOS test job.
...
```powershell
dotnet pack "Codebelt.Extensions.Newtonsoft.Json.slnx" --configuration Release --no-restore
```
...
By contributing to Extensions for Newtonsoft.Json API by Codebelt, you agree that your contributions will be licensed under the MIT license.
